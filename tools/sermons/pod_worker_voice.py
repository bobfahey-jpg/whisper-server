...
VOICE_CACHE = "/tmp/voice_cache"
_VOICE_SCRIPT = Path("/workspace/tools/sermons/sermon_voice.py")
...
def voice_consumer_thread():
    """Single background thread: scans VOICE_CACHE every 30s, runs voice analysis subprocess per MP3."""
    import glob, shutil
    log = make_log("voice-consumer")
    os.makedirs(VOICE_CACHE, exist_ok=True)
    log.info(f"Voice consumer started — watching {VOICE_CACHE}")
    active = {}  # slug -> Popen
    while True:
        # Reap completed subprocesses
        for slug in list(active):
            if active[slug].poll() is not None:
                log.info(f"VOICE done: {slug}")
                del active[slug]

        # Launch new ones (cap at 2 concurrent to avoid CPU saturation)
        if len(active) < 2:
            for mp3 in glob.glob(f"{VOICE_CACHE}/*.mp3"):
                slug = os.path.basename(mp3)[:-4]
                if slug not in active:
                    try:
                        proc = subprocess.Popen(
                            [sys.executable, str(_VOICE_SCRIPT),
                             "--slug", slug, "--mp3-path", mp3, "--delete-after"],
                            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                        )
                        active[slug] = proc
                        log.info(f"VOICE launched: {slug}")
                    except Exception as e:
                        log.warning(f"Voice launch failed for {slug}: {e}")
                        try: os.unlink(mp3)
...
def worker_thread(thread_num, stop_event, idle_event, t_start, max_secs, cpu_pool):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # Move MP3 to voice cache — consumer thread will analyze and delete it
            try:
                os.makedirs(VOICE_CACHE, exist_ok=True)
                cache_path = os.path.join(VOICE_CACHE, f"{slug}.mp3")
                os.rename(tmp_path, cache_path)
                tmp_path = None  # consumer owns the file now
            except Exception as e:
                log.warning(f"Voice cache move failed for {slug}: {e}")
